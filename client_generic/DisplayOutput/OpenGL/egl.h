...
public:
  ...
  friend void registry_handle_global(void *data, struct wl_registry *registry,
                                     uint32_t name, const char *interface,
                                     uint32_t version);
  friend void xdg_toplevel_configure_handler(void *data,
                                             struct xdg_toplevel *xdg_toplevel,
                                             int32_t width, int32_t height,
                                             struct wl_array *states);
  friend void xdg_toplevel_close_handler(void *data,
                                         struct xdg_toplevel *xdg_toplevel);
  friend void xdg_surface_configure_handler(void *data,
                                            struct xdg_surface *xdg_surface,
                                            uint32_t serial);
  friend void frame_configure(struct libdecor_frame *frame,
